...
//        thumbView.alpha = 0f
            ...
            // the expanded image.
            expandedImageCardView.setOnClickListener {
                currentAnimator?.cancel()

                // Animate the four positioning/sizing properties in parallel,
                // back to their original values.
                currentAnimator = AnimatorSet().apply {
                    play(ObjectAnimator.ofFloat(expandedImageCardView, View.X, startBounds.left)).apply {
                        with(ObjectAnimator.ofFloat(expandedImageCardView, View.Y, startBounds.top))
                        with(ObjectAnimator.ofFloat(expandedImageCardView, View.SCALE_X, startScale))
                        with(ObjectAnimator.ofFloat(expandedImageCardView, View.SCALE_Y, startScale))
                    }
                    duration = shortAnimationDuration.toLong()
                    interpolator = DecelerateInterpolator()
                    addListener(object : AnimatorListenerAdapter() {
                        ...
                        override fun onAnimationEnd(animation: Animator) {
                            thumbView.alpha = 1f
                            expandedImageCardView.visibility = View.GONE
                            currentAnimator = null
                        }
